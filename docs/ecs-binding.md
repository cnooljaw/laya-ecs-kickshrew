...
`board` 是 Scene/Map/Hole 的基础 ECS 层，位于 `src/game/board`：
...
- `HoleComponent`、`SceneComponent`、Entity 和 Projection 不从公开 `index.ts` 扩散。组合根从 `game/board/assembly.ts` 使用；具体测试直接 import `BoardComponents.ts`、`BoardEntities.ts` 或 `BoardProjection.ts`。

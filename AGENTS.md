...
- 改 dirty 同步：`src/sync/contracts/*ViewContract.ts`、`src/sync/DirtyFlags.ts`、`src/sync/rules/*ViewRules.ts`、`src/ecs/systems/DirtyMarkSystem.ts`、`src/binding/*ViewBinding.ts`
- 改独立玩法实体/怪物：`src/features/*`、`src/features/GameFeatures.ts`、对应 `src/sync/contracts/*ViewContract.ts`

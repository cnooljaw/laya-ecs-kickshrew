import { HammerComponent } from "../../../ecs/components";
import {
  BIT_HAMMER_HIT_FEEDBACK,
  BIT_HAMMER_HITTABLE,
  BIT_HAMMER_THUNDER,
  BIT_HAMMER_TYPE,
} from "../../DirtyFlags";
import type { IHammerNode } from "../../contracts/HammerViewContract";
import { createSyncRow, defineViewSyncSpec, noProjection } from "../ViewSyncSpec";

type HammerField = Extract<keyof typeof HammerComponent, string>;
const syncRow = createSyncRow<IHammerNode, HammerField>();

function applyHammerType({ eid, node }: { eid: number; node: IHammerNode }): void {
  node.setHammerType(HammerComponent.selectedType[eid]);
}

function applyThunderActive({ eid, node }: { eid: number; node: IHammerNode }): void {
  node.setThunderActive(HammerComponent.isThunderActive[eid] === 1);
}

function applyHitFeedback({ eid, node }: { eid: number; node: IHammerNode }): void {
  if (HammerComponent.hitSeq[eid] === 0) return;
  node.followTouch(HammerComponent.touchX[eid], HammerComponent.touchY[eid]);
  node.playHitAnimation();
}

export const HAMMER_VIEW_SYNC_SPEC = defineViewSyncSpec<IHammerNode, HammerField>(
  "HammerComponent",
  HammerComponent,
  [
    // bit                    label          fields              apply
    syncRow(BIT_HAMMER_TYPE,     "当前锤子类型", ["selectedType"],    applyHammerType),
    syncRow(BIT_HAMMER_THUNDER,  "雷神锤状态",   ["isThunderActive"], applyThunderActive),
    syncRow(BIT_HAMMER_HITTABLE, "是否可击打",   ["hitTable"],        noProjection),
    syncRow(BIT_HAMMER_HIT_FEEDBACK, "击打反馈", ["touchX", "touchY", "hitSeq"], applyHitFeedback),
  ],
);

...
import type { IPlayerHUD } from "../PlayerViewBinding";
...
export const PLAYER_VIEW_RULES = defineViewRules<IPlayerHUD, PlayerField>(
  ...
  [
    ...
    row<IPlayerHUD, PlayerField>(BIT_PLAYER_MONEY, "金币", ["money"],             applyMoney),
    row<IPlayerHUD, PlayerField>(BIT_PLAYER_ANGRY, "怒气", ["angry"],             applyAngry),
    row<IPlayerHUD, PlayerField>(BIT_PLAYER_POWER, "体力", ["power", "powerTop"], applyPower),
    row<IPlayerHUD, PlayerField>(BIT_PLAYER_LEVEL, "等级", ["level"],             applyLevel),
  ],
);

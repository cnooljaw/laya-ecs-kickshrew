...
local scheduler = require("network.scheduler") --modify by jaw
...
--自身扩展了EventProtocol类型,可以派发网络层的事件名称，实际上也为自己的一个观察者模式
...
function SocketTCP:close( ... )
	self.tcp:close()
    if self.connectTimeTickScheduler then Log.i("unscheduleGlobal") scheduler.unscheduleGlobal(self.connectTimeTickScheduler) end
...
function SocketTCP:_connect()
	...
	-- Log.i("SocketTCP._connect:", __succ, __status)
...
function SocketTCP:_onConnected()
	...
	local __tick = function()
		while true do
			...
			--Log.i("body:", __body, "__status:", __status, "__partial:", __partial)
    	    if __status == STATUS_CLOSED or __status == STATUS_NOT_CONNECTED then
               -- Log.i("SocketTCP:_onConnected")
		    	self:close()
		    	if self.isConnected then
		    		self:_onDisconnect()
		    	else 
